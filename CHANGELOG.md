...
+ [新增] 画布支持通过「媒体 URL」添加用户自有图床上的公网 HTTPS 图片或视频，并沿用生成节点的参考媒体提交流程。
+ [修复] 画布视频生成会提交组装提示词关联的参考图，并为 Waninter 视频请求优先保留参考图公网地址。
+ [优化] 画布与视频创作台的视频生成等待上限延长至 30 分钟，减少上游仍在生成时前端提前报超时的情况。
+ [新增] 渠道配置新增 Waninter API 预设，可通过生产 Token 拉取可用模型，并支持选择同步或异步生图及提交视频异步任务。
+ [修复] OpenAI 兼容的 GPT Image 与 DALL-E 模型会将自定义比例转换为各自支持的标准尺寸，避免请求因未配置尺寸而失败。
+ [修复] Waninter API 获取 Key 入口改为 classic 主题的 Token 管理页面。
+ [修复] 异步生图结果为跨域远程 URL 时仍会保存到生图工作台的生成记录。
+ [修复] 清理画布或资产图片时保留生图工作台生成记录仍在引用的本地图片。
+ [修复] Waninter 视频请求按 API 规范提交时长、分辨率和画面比例，并优先使用参考图公网地址。
+ [修复] Waninter 文本模型改用 `/v1/chat/completions`，避免误调用未实现的 Responses 接口。
+ [修复] Canvas Agent 按 Codex app-server 协议提交沙箱模式，修复新建或恢复会话被拒绝的问题。
...
+ [调整] Docker 部署文档改为 Nginx 静态前端说明，并支持通过绑定地址交由反向代理提供 HTTPS。
